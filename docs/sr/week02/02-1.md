...
lang-ref: ch.02-1
lecturer: Yann LeCun
title: Uvod u gradijentni spust i propagaciju unazad
...
## [Optimizacioni algoritam gradijentni spust](https://www.youtube.com/watch?v=d9vdh3b787Y&t=29s)
...
Parametarski modeli su samo funkcije koje zavise od ulaza i parametara koje možemo da obučimo. Ne postoji fundamentalna razlika između ta 2, osim što su obučljivi parametri deljeni između primera iz obučavajućeg skupa, dok ulaz varira od odbirka do odbirka. U većini radnih okvira dubokog učenja, parametri su implicitni, to jest, ne prosleđuju se kada se funkcija pozove. Oni su "sačuvani u funkciji", bar se tako može reći, makar u objektno-orijentisanim verzijama modela.

Parametarski modeli (funkcije) uzimaju ulaz, imaju vektor parametara i kreiraju izlaz. U nadgledanom učenju, ovaj izlaz ulazi u funkciju cene ($C(y,\bar{y}$)), koja poredi pravi izlaz (${y}$) sa izlazom modela ($\bar{y}$). Graf računanja ovog modela nalazi se na Slici 1.

| <center><img src="{{site.baseurl}}/images/week02/02-1/Figure1.jpg" alt="Figure1" style="zoom: 33%;" /></center> |
| <center>Slika 1: Prikaz grafa računanja parametarskog modela</center>|

Primeri parametarskih funkcija -

- Linearan model - Težinska suma komponenti ulaznog vektora:

  $$
  \bar{y} = \sum_i w_i x_i, C(y,\bar{y}) = \Vert y - \bar{y}\Vert^2
  $$

- Najbliži sused - Postoje ulaz x i matrica težina W sa svakim redom indeksiranim po k. Izlaz je vrednost k koja odgovara redu W koji je najbliži x:

  $$
  \bar{y} = \underset{k}{\arg\min} \Vert x - w_{k,.} \Vert^2
  $$

  Parametarski modeli takođe mogu da budu komplikovane funkcije.
<!--
...
-->
#### Blok dijagram notacija za graf računanja

-Promenljive (tenzor, skalar, kontinualni, diskretni)
...
- Determinističke funkcije

    <img src="{{site.baseurl}}/images/week02/02-1/deterministic_function.PNG" alt="deterministic_function" style="zoom:50%;" />

    - Imaju više ulaza i proizvode više izlaza. 
    - Imaju implicitnu parametarsku promenljivu (${w}$)
